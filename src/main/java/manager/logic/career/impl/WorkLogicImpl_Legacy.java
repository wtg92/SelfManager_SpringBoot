...
public class WorkLogicImpl_Legacy extends WorkLogic{
	...
	public synchronized void addItemToPlan(long adderId, long planId, String categoryName, int value, String note,
		...
		Plan existed = CacheScheduler_Old.getOne(CacheMode.E_ID, planId, Plan.class, ()->wDAO.selectExistedPlan(planId));
...
//			throw new LogicException(SMError.CANNOT_EDIT_OTHERS_PLAN);
		...
		CacheScheduler_Old.saveEntity(existed, p->wDAO.updateExistedPlan(p));
	}
	...
	public synchronized void addItemToWSPlan(long adderId, long wsId, String categoryName, int value, String note, PlanItemType type,
		...
		WorkSheet ws = CacheScheduler_Old.getOne(CacheMode.E_ID,wsId, WorkSheet.class, ()->wDAO.selectExistedWorkSheet(wsId));
		...
		CacheScheduler_Old.saveEntity(ws, w->wDAO.updateExistedWorkSheet(w));
	}
...
//			throw new LogicException(SMError.CANNOT_EDIT_OTHERS_PLAN);
	...
	public void resetPlanTags(long opreatorId, long planId, List<String> tags) throws SMException {
		Plan plan = CacheScheduler_Old.getOne(CacheMode.E_ID, planId, Plan.class, ()->wDAO.selectExistedPlan(planId));
		...
		CacheScheduler_Old.saveEntity(plan, p->wDAO.updateExistedPlan(p));
	}
	...
	public void resetWorkSheetTags(long opreatorId,long wsId,List<String> tags) throws SMException {
		...
		CacheScheduler_Old.saveEntity(ws, one->wDAO.updateExistedWorkSheet(one));
	}
...
//			throw new LogicException(SMError.CANNOT_EDIT_OTHERS_PLAN);
	...
	public void patchBalanceItem(long updaterId, int itemId, String name, double val)
		...
		PlanDept dept = CacheScheduler_Old.getOne(CacheMode.E_UNIQUE_FIELD_ID, updaterId, PlanDept.class, ()->wDAO.selectExistedBalanceByOwner(updaterId));
		WorkContentConverter.updatePlanDeptItem(dept, updaterId, itemId, name, val);
		CacheScheduler_Old.saveEntity(dept, w->wDAO.updateExistedBalance(w));
	}
...
//			throw new LogicException(SMError.CANNOT_EDIT_OTHERS_PLAN);
		...
		if(!existsWSBaseThisPlan) {
			CacheScheduler_Old.deleteEntityById(plan, (id)->wDAO.deleteExistedPlan(id));
			return;
		}
...
//			throw new LogicException(SMError.CANNOT_EDIT_OTHERS_PLAN);
	...
	public List<WorkSheet> loadWorkSheetInfosRecently(long opreatorId, int page) throws DBException, LogicException {
		...
		CacheScheduler_Old.saveInDBAndDeleteAllInCache(toSave, one->wDAO.updateExistedWorkSheet(one));
...
//		rlt.mood = calculateMoodByWorkItems(content.workItems);
		rlt.basePlanName = CacheScheduler_Old.getOne(CacheMode.E_ID, rlt.ws.getPlanId(), Plan.class, ()->wDAO.selectExistedPlan(rlt.ws.getPlanId())).getName();
		rlt.content = content;
	...
	public long loadWorkSheetCount(long loginerId,Calendar date) throws SMException {
		...
		long count = Long.parseLong(CacheScheduler_Old.getTempValOrInit(CacheMode.T_WS_COUNT_FOR_DATE, date, ()->wDAO.countWorkSheetByDate(date)));
		return count;
	}
	...
	public void calculatePlanStatesRoutinely(long loginId) {
		uL.checkPerm(loginId, SMPerm.SEE_SELF_PLANS);
			...
			CacheScheduler_Old.saveInDBAndDeleteAllInCache(stateChangedForPrepared, (p)->wDAO.updateExistedPlan(p));
			...
			CacheScheduler_Old.saveInDBAndDeleteAllInCache(stateChangedForActive, (p)->wDAO.updateExistedPlan(p));
			target.removeAll(stateChangedForActive);
			CacheScheduler_Old.putEntitiesToCacheById(target);
	}
	...
	public PlanBalanceProxy getBalance(long loginerId) throws DBException, LogicException {
		PlanDept dept = CacheScheduler_Old.getOneOrInitIfNotExists(CacheMode.E_UNIQUE_FIELD_ID, loginerId, PlanDept.class,
...
//		}
		...
		CacheScheduler_Old.deleteTempKey(CacheMode.T_WS_COUNT_FOR_DATE, ws.getDate());
	...
	public void deleteWorkSheet(long updaterId, long wsId) throws DBException, LogicException {
		...
		CacheScheduler_Old.deleteEntityById(ws, id->wDAO.deleteExistedWorkSheet(id));
		CacheScheduler_Old.deleteTempKey(CacheMode.T_WS_COUNT_FOR_DATE, ws.getDate());
	}
...
//		WorkContentConverter.updateWorkItem(ws, loginId, item.getId(), item.getValue(), item.getNote(), item.getMood(), item.isForAdd(), item.getStartTime(), item.getEndTime());
	...
	public synchronized void syncToBalance(long loginerId, long wsId, int planItemId) throws DBException, LogicException {
		...
		CacheScheduler_Old.saveEntity(ws, w->wDAO.updateExistedWorkSheet(w));
		CacheScheduler_Old.saveEntity(dept, d->wDAO.updateExistedBalance(d));
	}
	...
	public void syncPlanTagsToWorkSheet(long loginerId, long planId) throws SMException {
		Plan target = CacheScheduler_Old.getOne(CacheMode.E_ID, planId, Plan.class, ()->wDAO.selectExistedPlan(planId));
		...
		List<WorkSheet> toSave = wDAO.selectWorkSheetByField(SMDB.F_PLAN_ID, planId);
	 	...
	 	CacheScheduler_Old.saveInDBAndDeleteAllInCache(toSave, p->wDAO.updateExistedWorkSheet(p));
	}
	...
	public void copyPlanItemsFrom(long loginerId, long targetPlanId, long templetePlanId)
		...
		Plan target = CacheScheduler_Old.getOne(CacheMode.E_ID, targetPlanId, Plan.class, ()->wDAO.selectExistedPlan(targetPlanId));
		if(target.getOwnerId() != loginerId) {
//			throw new LogicException(SMError.CANNOT_EDIT_OTHERS_PLAN);
		}
		Plan templete = CacheScheduler_Old.getOne(CacheMode.E_ID, templetePlanId, Plan.class, ()->wDAO.selectExistedPlan(templetePlanId));
		...
		CacheScheduler_Old.saveEntity(target, p->wDAO.updateExistedPlan(p));
	}
