...
(function ($) {
  ...
  function createTemplate() {
    ...
    var templateSearch = searchable ? '<span class="dropdown-search">' + '<input type="text" class="search_input" placeholder="'+this.config.searchInputContext+'">' + '</span>' : '';

    if(isLabelMode){
      return '<div class="dropdown-display-label"><div class="dropdown-chose-list">' + templateSearch + '</div></div><div class="dropdown-main">{{ul}}</div>';
    }

    return this.isSingleSelect ? '<a href="javascript:;" class="dropdown-display"><span class="dropdown-chose-list"></span></a><div class="dropdown-main">' + templateSearch + '{{ul}}</div>'
      : '<a href="javascript:;" class="dropdown-display"><span class="dropdown-chose-list"></span><a href="javascript:;"  class="dropdown-clear-all">\xD7</a></a><div class="dropdown-main">' + templateSearch + '{{ul}}</div>'
      ;
  }
  ...
  function parseObjToLis(liObjs,attrs){
    ...
    for(let i=0;i<liObjs.length;i++){
      ...
      $li.attr({
        "tabindex":i,
        "data-value" : val
      }).addClass("dropdown-option").text(val);
      attrs.forEach(e=>{
        $li.attr(e,li[e]);
      })
      result += $li.get(0).outerHTML;
    }
    return result;
  }
  ...
  var action = {
    ...
    search: throttle(function (event) {
      ...
      var data = _dropdown.config.liObjs;
      var result = [];
      ...
      $.each(data, function (key, value) {
        if (value.val.toLowerCase().indexOf(intputValue) > -1 || '' + value.id === '' + intputValue) {
          result.push(value);
        }
      });
      $el.find('ul').html(parseObjToLis(result,_config.attrs) || _config.searchNoData,_dropdown);
    }, 300),
    ...
    singleChoose: function singleChoose(event) {
      ...
      let tableIndex = parseInt($target.attr("tabindex"));

      let targetLi = _config.liObjs[tableIndex];
      ...
      let $showText = generateShowText(targetLi.val);

      _dropdown.$choseList.html($showText);
      _config.choice(targetLi,$showText);
    },
  ...
  Dropdown.prototype = {
    ...
    renderSelect: function renderSelect(isUpdate, isCover) {
      var _this = this;
      var $el = _this.$el;
      var elemLi = parseObjToLis(_this.config.liObjs,_this.config.attrs);
      var template;
      ...
      let selectedLi =  _this.config.liObjs.filter(e=>e.selected)[0]

      let $defaultHTML = generateShowText(selectedLi.val);
      _this.$choseList.prepend($defaultHTML);
    },
    bindEvent: function bindEvent() {
      ...
      if (_this.isLabelMode) {
        ...
        if(!_this.isSingleSelect){
          ...
          $el.on("click", '.dropdown-display', $.proxy(action.show, _this));
        }
      ...
      // 搜索
      $el.on(EVENT_SPACE.keyup, 'input.search_input', $.proxy(action.search, _this));
  ...
  function generateShowText(text){
    let $defaultHTML = $("<span>");
    $defaultHTML.addClass("dropdown-selected").text(text);
    return $defaultHTML;
  }
